...
class PackedConformationDataset(ConformationDataset):     # 按构像构成数据，而非坐标，添加两个新属性pos_ref, num_pos_ref，test共1000分子，构像数目不止目前这些
    ...
    def _pack_data_by_mol(self):
        ...
        """
        self._packed_data = defaultdict(list)            # 将 test_set合并为一个self._packed_data, 由构象集合--分子集合
        ...
        for k, v in self._packed_data.items():
            ...
            data.pos_ref = torch.cat(all_pos, 0) # (num_conf*num_node, 3)             构象坐标
            data.num_pos_ref = torch.tensor([len(all_pos)], dtype=torch.long)       # 构象数目
            #del data.pos

            if hasattr(data, 'totalenergy'):            # 删除属性 totalenergy, boltzmannweight
