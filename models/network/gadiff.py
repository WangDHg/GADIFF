...
from ..encoder import SchNetEncoder, GEncoder, get_edge_encoder, EGNN
...
class GAEpsNetwork(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.noise_encoder = EGNN(
            in_node_nf=self.hidden_dim, 
            in_edge_nf=self.hidden_dim, 
            ...
            node_config=config.edge_type_func,
            config=config,
        )
    ...
    def forward(self, atom_type, pos, bond_index, bond_type=None, batch=None, time_step=None, 
                edge_index=None, edge_type=None, edge_length=None, return_edges=False, 
                extend_order=True, extend_radius=True, is_sidechain=None, angle_edge_index=None, torsion_edge_index=None):
        """
        Args:
        ...
        if edge_index is None or edge_type is None or edge_length is None:
            edge_index, edge_type, angle_edge_mask, torsion_edge_mask, radius_edge_mask = extend_graph_order_radius(
                num_nodes=N,
                pos=pos,
        ...
        bond_edge_mask = is_bond(edge_type)
        # angle_edge_mask = is_angle(edge_type)
        # torsion_edge_mask = is_torsion(edge_type)
        ...
        noise, _, _ = self.noise_encoder(
            atom_type, pos, edge_index, edge_type, node_mask=None, 
            edge_mask=[bond_edge_mask, angle_edge_mask, torsion_edge_mask, radius_edge_mask],
            node_type=None, edge_length=edge_length, N=N)
        
        return noise, edge_index, edge_length
        
    
    def get_loss(self, atom_type, pos, bond_index, bond_type=None, batch=None, num_nodes_per_graph=None, num_graphs=None, 
    ...
    def get_loss_diffusion(self, atom_type, pos, bond_index, bond_type=None, batch=None, num_nodes_per_graph=None, num_graphs=None, 
                 anneal_power=2.0, return_unreduced_loss=False, return_unreduced_edge_loss=False, extend_order=True, extend_radius=True, is_sidechain=None):
        N = atom_type.size(0)
        node2graph = batch
        ...
        node_eq_pos, edge_index, edge_length = self(
            atom_type = atom_type,
            pos = pos_perturbed,                 # noised_pos
        ...
        d_perturbed = edge_length                            # (E, 1)  add noise
        ...
        target_eq_pos = eq_transform(d_target, pos_perturbed, edge_index, edge_length)  # (N, 3)
        
        loss = torch.sum((node_eq_pos - target_eq_pos)**2, -1, keepdim=True)
    ...
    def langevin_dynamics_sample(self, atom_type, pos_init, bond_index, bond_type, batch, num_graphs, extend_order, extend_radius=True, 
                                 n_steps=100, sampling_steps=10, step_lr=0.0000010, clip=1000, clip_local=None, clip_pos=None, min_sigma=0, is_sidechain=None,
                                 global_start_sigma=float('inf'), w_global=0.2, w_reg=1.0, angle_edge_index=None, torsion_edge_index=None, **kwargs):
        if self.model_type == 'diffusion':
            return self.langevin_dynamics_sample_diffusion(atom_type, pos_init, bond_index, bond_type, batch, num_graphs, extend_order, extend_radius, 
                        n_steps, sampling_steps, step_lr, clip, clip_local, clip_pos, min_sigma, is_sidechain,
                        global_start_sigma, w_global, w_reg, angle_edge_index=angle_edge_index, torsion_edge_index=torsion_edge_index,
    ...
    def langevin_dynamics_sample_diffusion(self, atom_type, pos_init, bond_index, bond_type, batch, num_graphs, extend_order, extend_radius=True, 
        ...
        with torch.no_grad():
            ...
            step_length = int(n_steps/sampling_steps)
            seq = range(self.num_timesteps-n_steps, self.num_timesteps, step_length)
            ...
            for i, j in tqdm(zip(reversed(seq), reversed(seq_next)), desc='sample'):
                ...
                if sampling_type == 'generalized' or sampling_type == 'ddpm_noisy':
                    ...
                    if sampling_type == 'generalized':
                        ...
                        step_size_pos_generalized = ((1 - at).sqrt() / at.sqrt() - c2 / at_next.sqrt())
                        ...
                        step_size_noise_generalized = (c1 / at_next.sqrt())
                        ...
                        beta_t = 1 - at / atm1
                        e = -eps_pos
                        pos0_from_e = (1.0 / at).sqrt() * pos - (1.0 / at - 1).sqrt() * e  # X0
                        mean_eps = (
                    ...
                    pos_next = pos + step_size * eps_pos / sigmas[i] + noise * torch.sqrt(step_size*2)  # Langevin Dynamics
    ...
    def get_loss_dsm(self, atom_type, pos, bond_index, bond_type, batch, num_nodes_per_graph, num_graphs, 
        ...
        noise_levels = self.sigmas.index_select(0, time_step)  # (G, )
        # Perterb pos
        sigmas_pos = noise_levels.index_select(0, node2graph).unsqueeze(-1)  # (E, 1)？ （N,1)
        pos_noise = torch.zeros(size=pos.size(), device=pos.device)
        pos_noise.normal_()
...
def is_bond(edge_type):
    return torch.mul((edge_type > 0), (edge_type < (torch.max(edge_type)-2)))

def is_angle(edge_type, max_type):
    return edge_type == (max_type -1)

def is_torsion(edge_type, max_type):
    return edge_type == max_type
