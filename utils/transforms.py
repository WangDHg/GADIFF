...
class AddHigherOrderEdges(object):
    ...
    def get_higher_order_adj_matrix(self, adj, order):
        ...
        adj_order_mats = [self.binarize(adj)]
        zeros = torch.zeros_like(adj)

        for i in range(1, order):
            adj_temp = self.binarize(adj_order_mats[i-1] @ adj_order_mats[0])
    ...
    def __call__(self, data: Data):
        ...
        adj_mats, adj_order = self.get_higher_order_adj_matrix(adj, self.order)  # (N, N)
