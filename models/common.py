...
class MultiLayerPerceptron(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dims, activation="relu", dropout=0):
        ...
        # self.batchnorm = nn.BatchNorm1d(self.dims[0])
    ...
    def forward(self, input):
        ...
        for i, layer in enumerate(self.layers):
            x = layer(x)
            if i < len(self.layers) - 1 and (i+1)%2 == 0:
                if self.activation:
                    x = self.activation(x)
                if self.dropout:
                    x = self.dropout(x)
...
def _extend_graph_order(num_nodes, edge_index, edge_type=None, order=3):
    """
    Args:
    ...
    def get_higher_order_adj_matrix(adj, order):
        ...
        # return order_mat
        # adj_order_mats
        adj_order_mats = [binarize(adj)]
        zeros = torch.zeros_like(adj)

        for i in range(1, order):
            adj_temp = binarize(adj_order_mats[i-1] @ adj_order_mats[0])
            adj_temp = adj_temp - torch.eye(adj.size(0), dtype=torch.long, device=adj.device)
            adj_temp = torch.where(torch.lt(adj_temp, 0), zeros, adj_temp)
            adj_order_mats.append(adj_temp)
        
        # order_mat generate 
        order_mat = adj_order_mats[0] + \
                    torch.where(torch.lt(adj_order_mats[1]-adj_order_mats[0], 0), zeros, (adj_order_mats[1]-adj_order_mats[0])) * 2 + \
                    torch.where(torch.lt(adj_order_mats[2]-adj_order_mats[1]-adj_order_mats[0], 0), zeros, (adj_order_mats[2]-adj_order_mats[1]-adj_order_mats[0])) * 3

        
        return adj_order_mats, order_mat
    ...
    adj_mats, adj_order = get_higher_order_adj_matrix(adj, order)  # (3, N, N)
    angle_adj = adj_mats[1]
    torsion_adj = adj_mats[2]
    angle_edge_index, _ = dense_to_sparse(angle_adj)
    torsion_edge_index, _ = dense_to_sparse(torsion_adj)

    if edge_type != None:
        type_mat = to_dense_adj(edge_index, edge_attr=edge_type).squeeze(0)   # (N, N)
    else:
        type_mat = to_dense_adj(edge_index).squeeze(0)
    ...
    return new_edge_index, new_edge_type, angle_edge_index, torsion_edge_index
    

def _extend_to_radius_graph(pos, edge_index, edge_type=None, cutoff=10, batch=None, unspecified_type_number=0, angle_edge_index=None, torsion_edge_index=None, is_sidechain=None):
    ...
    if min(edge_index.size()) != 0:
        composed_adj = (bgraph_adj + rgraph_adj).coalesce()  # Sparse (N, N, T)
    else:
        composed_adj = rgraph_adj.coalesce()
    ...
    return new_edge_index, new_edge_type, rgraph_edge_index
    
    # return new_edge_index, new_edge_type
    
    # # rgraph_edge_mask
    # if min(edge_index.size()) != 0:
    #     edge_index_ones = torch.sparse.LongTensor(
    #         new_edge_index,
    #         torch.ones(new_edge_index.size(1)).long().to(pos.device),
    #         torch.Size([N,N])
    #     )
    #     rgraph_ones = torch.sparse.LongTensor(
    #         rgraph_edge_index,
    #         torch.ones(rgraph_edge_index.size(1)).long().to(pos.device),
    #         torch.Size([N, N])
    #     )
    #     angle_ones = torch.sparse.LongTensor(
    #         angle_edge_index,
    #         torch.ones(angle_edge_index.size(1)).long().to(pos.device),
    #         torch.Size([N, N])
    #     )
    #     torsion_ones = torch.sparse.LongTensor(
    #         torsion_edge_index,
    #         torch.ones(torsion_edge_index.size(1)).long().to(pos.device),
    #         torch.Size([N, N])
    #     )
        
    #     radius_g = (edge_index_ones+rgraph_ones).coalesce()
    #     ones_index_radius = radius_g.indices()
    #     ones_type_radius = radius_g.values().long()
    #     rgraph_edge_mask = (ones_type_radius == 2)
        
    #     angle_g = (edge_index_ones+angle_ones).coalesce()
    #     ones_index_angle = angle_g.indices() 
    #     ones_type_angle = angle_g.values().long()
    #     angle_edge_mask = (ones_type_angle == 2)
        
    #     torsion_g = (edge_index_ones+torsion_ones).coalesce()
    #     ones_index_torsion = torsion_g.indices()
    #     ones_type_torsion = torsion_g.values().long()
    #     torsion_edge_mask = (ones_type_torsion == 2)
        
    # else:
    #     rgraph_edge_mask = [True]*new_edge_type.size(1)
    #     angle_edge_mask = [False]*new_edge_type.size(1)
    #     torsion_edge_mask = [False]*new_edge_type.size(1)
    
    # return new_edge_index, new_edge_type, angle_edge_mask, torsion_edge_mask, rgraph_edge_mask


def extend_graph_order_radius(num_nodes, pos, edge_index, edge_type=None, batch=None, order=3, cutoff=10.0, 
                              extend_order=True, extend_radius=True, is_sidechain=None, angle_edge_index=None, 
                              torsion_edge_index=None):
    
    bond_size = edge_index.size(0)
    N = pos.size(0)
    
    if extend_order and min(edge_index.size()) != 0:
        edge_index, edge_type, angle_edge_index, torsion_edge_index = _extend_graph_order(
            num_nodes=num_nodes, 
    ...
    if extend_radius:
        edge_index, edge_type, rgraph_edge_index = _extend_to_radius_graph(
            pos=pos,
            edge_index=edge_index, 
            edge_type=edge_type, 
            cutoff=cutoff, 
            ...
            torsion_edge_index=torsion_edge_index
        )
    ...
    if bond_size != 0:
        edge_index_ones = torch.sparse.LongTensor(
            edge_index,
            torch.ones(edge_index.size(1)).long().to(pos.device),
            torch.Size([N,N])
        )
        angle_ones = torch.sparse.LongTensor(
            angle_edge_index,
        ...
        if extend_radius:
            ...
            ones_index_radius = radius_g.indices()
            ones_type_radius = radius_g.values().long()
            radius_edge_mask = (ones_type_radius == 2)
            
    else:
        angle_edge_mask = [False]*edge_type.size(0)
        torsion_edge_mask = [False]*edge_type.size(0)
        if extend_radius:
            radius_edge_mask = [True]*edge_type.size(0)
    
    if extend_radius:
        return edge_index, edge_type, angle_edge_mask, torsion_edge_mask, radius_edge_mask
    else:
        return edge_index, edge_type, angle_edge_mask, torsion_edge_mask
