...
class ShiftedSoftplus(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        # return F.leaky_relu(x) - self.shift
        # return F.relu(x)


class CFConv(MessagePassing):             # MPNN
    ...
    def forward(self, x, edge_index, edge_length, edge_attr):
        if self.smooth:          # another activation
            C = 0.5 * (torch.cos(edge_length * PI / self.cutoff) + 1.0)     # 将 (edge_length/cutoff)置于cos曲线的PI范围内，将其范围由-1~1修改至0~2而后除2再转换至范围0~1内。
            C = C * (edge_length <= self.cutoff) * (edge_length >= 0.0)     # Modification: cutoff 保留 0-cutoff内的值
        else:
...
class SchNetEncoder(Module):
    ...
    def __init__(self, hidden_channels=128, num_filters=128,
        ...
        self.embedding = Embedding(119, hidden_channels, max_norm=10.0)
    ...
    def forward(self, z, edge_index, edge_length, edge_attr, embed_node=True):
        if embed_node:
            assert z.dim() == 1 and z.dtype == torch.long         # [17158,]
            h = self.embedding(z)
        else:
            h = z                       # [17158, 128]
        for interaction in self.interactions:
            h = h + interaction(h, edge_index, edge_length, edge_attr)     # h是节点特征
            # h = self.act_layer(h + interaction(h, edge_index, edge_length, edge_attr))     # h是节点特征

        return h
    

class SchNetConv(Module):
    ...
    def forward(self, h, edge_index, edge_length, edge_attr):
        # [17158, 128]
        for interaction in self.interactions:
            h = h + interaction(h, edge_index, edge_length, edge_attr)     # h node feature
