...
class MLPEdgeEncoder(Module):
    ...
    def __init__(self, hidden_dim=100, activation='relu'):
        ...
        self.bond_emb = Embedding(26, embedding_dim=self.hidden_dim)
        # self.mlp = MultiLayerPerceptron(1, [self.hidden_dim, self.hidden_dim], activation=activation)
        self.mlp = MLP([1, self.hidden_dim, self.hidden_dim], act=activation, is_res=True)
        # self.mlp = MLP([1, self.hidden_dim, self.hidden_dim * 2, self.hidden_dim * 2, self.hidden_dim], act=activation)   # 4层
        # self.mlp = MLP([1, self.hidden_dim, self.hidden_dim, self.hidden_dim, self.hidden_dim], act=activation, is_res=True)   # 4层
